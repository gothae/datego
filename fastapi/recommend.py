...
def category_filter(dataFrame, spot_category_detail, categories):
    ...
    for idx, row in dataFrame.iterrows():
        spot_id = row['id']
        spot_categories = spot_category_detail[spot_category_detail['spot_id']
                                               == spot_id]['category_detail_id'].values.tolist()
...
def compute_cos_sim(spots, arr):
    ...
    spotId = {}
    for idx, c in enumerate(spots['name']):
        spotId[idx] = c
    id2spot = {}
    for idx, c in spotId.items():
        id2spot[c] = idx
...
def train_model(reviews):
    ...
    data = Dataset.load_from_df(
        reviews[['user_id', 'spot_id', 'rate']], reader)
...
def rec_spot(cbf_return, cf_return):
    ...
    rec_for_user = sorted(rec_for_user.items(),
